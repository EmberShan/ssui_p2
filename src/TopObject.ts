    ...
    public get allowAsyncDamageRedraw() {
    ...
    public layoutAndDrawAll(): void {
        ...
        if (this.damaged) {
            console.log(
            "Improper damage declaration during redraw detected " +
    ...
    private _getCanvasContext(canvasID: string): CanvasRenderingContext2D {
        ...
        throw new Error(
            `Can't get rendering context for canvas element with id:"${canvasID}"`
        );
